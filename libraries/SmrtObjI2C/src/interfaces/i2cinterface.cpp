...
namespace smrtobj
{
  ...
  namespace i2c
  {
    ...
    I2CInterface::I2CInterface(uint8_t addr) :
        m_device_addr(addr)
    {
      m_type = TYPE_BIDIRECTIONAL;
    }
    ...
    int8_t I2CInterface::readAllBytes(uint8_t devAddr, uint8_t length,
        uint8_t *data, uint16_t timeout)
    {
      ...
      {
        #if ((ARDUINO < 100) || I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_NBWIRE)
    ...
    /*
     ...
     for (; Wire.available() && (timeout == 0 ||  t.residualTime(timeout) == 0); count++) {
     #if ((ARDUINO < 100) || I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_NBWIRE)
     data[count] = Wire.receive();
     #elif (ARDUINO >= 100)
     data[count] = Wire.read();
     ...
     if (timeout > 0 &&  t.residualTime(timeout) == 0 && count < length) count = -1; // timeout
     ...
     */
    bool I2CInterface::writeAllBytes(uint8_t devAddr, uint8_t length,
        uint8_t *data, uint16_t timeout)
    {
#ifdef I2CDEV_SERIAL_DEBUG
      Serial.print("I2C (0x");
